...
class Error extends Common
{
    ...
    public function index($step = 0)
    {
        ...
        // 检测PHP环境
        if(version_compare(PHP_VERSION,'7.0','<'))  die('PHP版本过低，最少需要PHP 7，请升级PHP版本！');
    ...
    private function step5()
    {
        ...
        if (file_exists($sqlFile)) {
            ...
            $sqlList = parse_sql($sql, 0, ['mgcms_' => $config['prefix']]);
...
INFO;
        file_put_contents(Env::get('config_path').'mg_auth.php', $mg_auth);


        //云平台密匙
        $identifier = password_hash($_SERVER['HTTP_HOST'], PASSWORD_DEFAULT);
        $no=md5($identifier);
        $mg_auth = <<<INFO
<?php
/// 请妥善保管此文件，谨防泄漏
return ['identifier' => '{$identifier}', 'no' => '{$no}'];

INFO;
        file_put_contents(Env::get('config_path').'mg_cloud.php', $mg_auth);
