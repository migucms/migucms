...
class Store extends Admin
{
    ...
    protected function initialize()
    {
        ...
        $this->cloud    = new Cloud(config('mg_cloud.identifier'), $this->tempPath);
    }
    ...
    private function _moduleInstall($appName, $appKeys, $file, $unzipPath)
    {
        ...
        $result = ModuleModel::install($result->id, 1);
        if ($result !== true) {
    ...
    private function _pluginsInstall($appName, $appKeys, $file, $unzipPath)
    {
        ...
        if (!is_dir($this->rootPath.'plugins/'.$appName)) {
            ...
            // 复制静态资源
            Dir::copyDir($staticPath, './static/plugins/'.$appName);
        } else {
            return '已存在同名插件';
        }
        ...
        $result = PluginsModel::install($result->id);
        if ($result !== true) {
