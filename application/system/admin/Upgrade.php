...
class Upgrade extends Admin
{
    ...
    protected function initialize() {
        ...
        $this->cloud            = new Cloud(config('mg_cloud.identifier'), $this->updatePath);
    ...
    public function index()
    {
        if ($this->request->isPost()) {
            if (empty(config('mg_cloud.no'))) {
                return $this->error(base64_decode('6K+36YeN5paw5LiL6L295qGG5p62'));
            }
            ...
            $data['version']    = config('mgcms.version');
            
            $res = $this->cloud->data($data)->api('bind');

            if (isset($res['code']) && $res['code'] == 1) {
                // 缓存站点标识
                $file = Env::get('config_path').'mg_cloud.php';
                $str = "<?php\n// 请妥善保管此文件，谨防泄漏\nreturn ['identifier' => '".$res['data']."', 'no' => '".config('mg_cloud.no')."'];\n";
    ...
    public function download($version = '')
    {
        ...
        if (!is_file($lock)) {
            ...
            return $this->error('升级任务执行中，请手动删除文件锁！<br>文件地址：/backup/uppack/'.$this->identifier.'upgrade.lock');
        }
    ...
    private function _systemInstall($file, $version)
    {
        ...
        $backPath = $this->updateBackPath.config('mgcms.version').'/';
        if (!is_dir($backPath)) {
        ...
        if (isset($upInfo['delete'])) {
            foreach ($upInfo['delete'] as $k => $v) {
                ...
                } else if (is_dir($this->rootPath.$v)) {
                    @rmdir($this->rootPath.$v);
                }
            }
        }
        ...
        if (is_file($sqlFile)) {
            ...
            $sqlList = parse_sql($sql, 0, ['mgcms_' => config('database.prefix')]);
            if ($sqlList) {
